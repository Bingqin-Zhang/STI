...
int* char_to_bin(char c, int taille){
    int* rst = int_to_bin(c, taille); // On converti notre caractère en binaire
    return rst;
}
...
void afficher_cle(int* cle, int n){
	int i;
	for(i = 0; i<n; i++){
		printf("%d",cle[i]); // Ici on affiche la cle (le bloc de n bits)
	}
	printf("\n");
}
...
int* Fk(int* cle, int* k, int* P4, int* EP){
    ...
    return cle; // On retourne donc notre cle ainsi modifié
}
...
int* get_k2(int* key, int* P8, int* P10, int* C_SHIFT){
    return permute(permute(permute(permute(permute(key,10,P10),10,C_SHIFT),10,C_SHIFT),10,C_SHIFT),8,P8); // On génère ici la sous cle k2 avec la permutation 10, trois rotations circulaire, et la permutation 8
}
